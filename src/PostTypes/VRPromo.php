...
class VRPromo extends PostTypeSimple
{
  ...
  {
    parent::register();
    add_action(Actions::addMetaBoxes(), [$this, 'rzf_add_promo_code_meta']);
    add_action(Actions::savePost(), [$this, 'rzf_save_promo_code_meta'], 10, 2);
    add_action(Actions::addMetaBoxes(), [$this, 'rzf_add_promo_listing_meta']);
    add_action(Actions::savePost(), [$this, 'rzf_save_promo_listing_meta'], 10, 2);
  }
  ...
  public function getPostTypeDefinition(): array
  {
    return [
      ...
      'show_in_rest' => true,
      'rewrite' => array('with_front' => false, 'slug' => get_rezfusion_option(Options::customPromoSlug()) ?: 'specials')
    ];
  }
  ...
  public function rzf_promo_listing_meta_box($post)
  {
    ...
    $meta_key = Metas::promoListingValue();
...
?>
  ...
  { ?>
    <p><input type="text" name="promo-code-input" id="promo-code-input" value="<?php echo esc_attr(get_post_meta($post->ID, Metas::promoCodeValue(), true)); ?>"></p>
<?php }
